  ...
  $.TokenList = function (input, url_or_data, settings) {
    ...
    var input_box = $("<input type=\"text\"  autocomplete=\"off\">").css({
      ...
      if (settings.tokenLimit === null || settings.tokenLimit !== token_count) {
        $(this).closest('.token-input-list').addClass('token-input-list-focused');
        show_dropdown_hint();
      }
    }).blur(function () {
    $(this).closest('.token-input-list').removeClass('token-input-list-focused');
      hide_dropdown();
      ...
      case KEY.DOWN:
        if (!$(this).val()) {
          ...
          if (event.keyCode === KEY.DOWN || event.keyCode === KEY.RIGHT) {
            ...
            if ($('li', dropdown).index(selected_dropdown_item) === 0) {
              deselect_dropdown_item($(selected_dropdown_item));
              $('.token-input-input-token input').trigger('focus');
              return false;
             }
          }
